<?php
/**
 * `wp dukandaar rag-spike`, the RAG Phase 0 decision-gate runner (S0.5).
 *
 ...
 *
 *   wp dukandaar rag-spike                       # offline: canned embeddings
 *   wp dukandaar rag-spike --k=3 --sizes=1000,5000,20000
 *   wp dukandaar rag-spike --report=/path.md     # write the report somewhere else
 *
...
final class Dukandaar_Rag_Spike_CLI {
	...
	public function __invoke( array $args, array $assoc ): void {
		...
		$path = trailingslashit( $upload['basedir'] ) . 'dukandaar-ai-shopping-assistant-for-woocommerce/' . $name;

		[ $texts, $vecs, $queries, $mode ] = $this->build_dataset();

		\WP_CLI::log( "RAG spike, relevance ({$mode}) at recall@{$k}:" );
		$cmp = Dukandaar_Rag_Spike::compare( $texts, $vecs, $queries, $k );
		...
		foreach ( $sizes as $size ) {
			$stats     = Dukandaar_Rag_Spike::scan_latency( $size, 512, 7 );
			$latency[] = $stats;
	...
	private function live_dataset( string $key ): ?array {
		...
		foreach ( $products as $p ) {
			...
			$docs[ $id ]  = Dukandaar_Embedding_Document::compose(
				[
					'title'             => $p->get_name(),
	...
	private function render_report( array $cmp, array $latency, int $k, string $mode ): string {
		...
		return "# RAG Spike Report (Phase 0)\n\n"
			. "Generated by `wp dukandaar rag-spike`. Dataset: **{$mode}**.\n\n"
			. "## Verdict\n\n{$verdict}\n\n"
			. "## Relevance (recall@{$k})\n\n"
			...
			. "## Notes\n\n"
			. "- Offline/canned mode validates the harness and the keyword/vector/hybrid logic deterministically. Run with an OpenAI key configured (`dukandaar_openai_api_key`) for the real-embedding confirmation on the live catalog.\n"
			. "- This is a Phase 0 spike: no shipped tool, UI, or release. The primitives exercised here (`Dukandaar_Vector_Math`, `Dukandaar_Rrf`, `Dukandaar_Embedding_Document`, `Dukandaar_Relevance_Metrics`, `Dukandaar_Rag_Spike_Retriever`) are what Phase 1 (S1.x) composes.\n";
	}
}

\WP_CLI::add_command( 'dukandaar rag-spike', Dukandaar_Rag_Spike_CLI::class );
