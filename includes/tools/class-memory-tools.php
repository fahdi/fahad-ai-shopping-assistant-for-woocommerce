 ...
 *
 * A drop-in feature pack (same pattern as Dukandaar_Order_Tools / Dukandaar_Wallet_Tools):
 * a self-contained class in its own file under includes/tools/ that self-registers a
 * provider at the bottom via Dukandaar_Tool_Registry::register_pack(), AND hooks the
 * `dukandaar_system_prompt` filter for context injection. The bootstrap (and the test
 ...
 *
 * Consent is a boolean flag in the DUKANDAAR_OPTIN_META user-meta key, set ONLY by
 ...
 *
 * inject_preferences() is hooked to the `dukandaar_system_prompt` filter (issue #20), so
 ...
 *      Dukandaar_Tool_Registry::dispatch() runs Dukandaar_Auth::guard_logged_in() BEFORE
 *      the callback, a guest is blocked centrally and the callback (and user meta) is
 *      never reached.
 *   2. CURRENT-USER-ONLY. Every read/write targets Dukandaar_Auth::current_user_id();
 ...
 */
final class Dukandaar_Memory_Tools {
	 ...
	 * current user's meta directly and use the shared Dukandaar_Auth boundary.
	 *
	 ...
	 *
	 * Writes the boolean flag for Dukandaar_Auth::current_user_id() ONLY, a model-supplied
	 ...
	 * Dukandaar_Auth::current_user_id() (a model-supplied user id is ignored), and a new
		...
		if ( ! array_key_exists( $key, $prefs ) && count( $prefs ) >= self::MAX_PREFERENCES ) {
			return [
				'error' => sprintf(
					...
					__( 'I can only remember up to %d preferences. Please clear some before adding more.', 'dukandaar-ai-shopping-assistant-for-woocommerce' ),
	 ...
	 * exactly what is remembered. Reads ONLY Dukandaar_Auth::current_user_id(), a
	 ...
	 * optionally withdraw consent too. Acts on Dukandaar_Auth::current_user_id() ONLY, a
	...
	public static function register_exporter( array $exporters ): array {
		$exporters[ self::PRIVACY_KEY ] = [
			'exporter_friendly_name' => __( 'Dukandaar assistant memory', 'dukandaar-ai-shopping-assistant-for-woocommerce' ),
	...
	public static function register_eraser( array $erasers ): array {
		$erasers[ self::PRIVACY_KEY ] = [
			'eraser_friendly_name' => __( 'Dukandaar assistant memory', 'dukandaar-ai-shopping-assistant-for-woocommerce' ),
	...
	public static function gdpr_export( string $email_address, int $page = 1 ): array {
		...
		$items = [
			[
				'name'  => __( 'Memory consent', 'dukandaar-ai-shopping-assistant-for-woocommerce' ),
				'value' => self::has_consent( $user_id )
					? __( 'Yes', 'dukandaar-ai-shopping-assistant-for-woocommerce' )
					: __( 'No', 'dukandaar-ai-shopping-assistant-for-woocommerce' ),
			],
		];
		...
		return [
			'data' => [
				[
					...
					'group_label' => __( 'Assistant memory (saved preferences)', 'dukandaar-ai-shopping-assistant-for-woocommerce' ),
	...
	// -------------------------------------------------------------------------
	// Context injection (dukandaar_system_prompt filter), loop-free.
	// -------------------------------------------------------------------------

	/**
	 * Append a compact preferences block to the system prompt for a logged-in, opted-in
	 * customer with stored preferences. Hooked to the `dukandaar_system_prompt` filter
	...
	public static function inject_preferences( string $prompt ): string {
		if ( ! Dukandaar_Auth::is_logged_in() ) {
			return $prompt;
		}
...
// `dukandaar_system_prompt` filter (issue #20). This is how memory is injected WITHOUT
...
if ( function_exists( 'add_filter' ) ) {
	add_filter( 'dukandaar_system_prompt', [ 'Dukandaar_Memory_Tools', 'inject_preferences' ] );
	...
	add_filter( 'wp_privacy_personal_data_exporters', [ 'Dukandaar_Memory_Tools', 'register_exporter' ] );
	add_filter( 'wp_privacy_personal_data_erasers', [ 'Dukandaar_Memory_Tools', 'register_eraser' ] );
}
// @codeCoverageIgnoreEnd
