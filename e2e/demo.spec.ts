...
// ─────────────────────────────────────────────────────────────────────────────
// Live-demo deep link. Arriving via ?dukandaar_demo=<question> auto-opens the panel,
...
// already answering. ?dukandaar_demo=1 uses the built-in default question.
// ─────────────────────────────────────────────────────────────────────────────

test.describe('live-demo deep link', () => {
	test.use({ viewport: { width: 1280, height: 800 } });

	test('?dukandaar_demo=<question> auto-opens, types and sends the question', async ({ page }) => {
		await mockApi(page);
		const q = 'What wireless headphones do you have and how much?';
		await page.goto('/e2e/harness/?dukandaar_demo=' + encodeURIComponent(q));
	...
	test('?dukandaar_demo=1 sends the built-in default question', async ({ page }) => {
		await mockApi(page);
		await page.goto('/e2e/harness/?dukandaar_demo=1');
		await page.waitForFunction(() => !!document.getElementById('chatbot-toggle'));
		await expect(page.locator('#chatbot-panel')).not.toHaveClass(/chatbot-hidden/);
	...
	test('no ?dukandaar_demo param leaves the widget closed', async ({ page }) => {
		await mockApi(page);
