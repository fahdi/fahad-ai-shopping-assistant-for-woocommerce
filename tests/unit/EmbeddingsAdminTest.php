...
class EmbeddingsAdminTest extends TestCase {
	...
	public function test_save_persists_sanitized_settings(): void {
		Dukandaar_Embeddings_Admin::save(
			[
		...
		$this->assertSame( 1, $this->options[ Dukandaar_Embeddings_Admin::OPT_ENABLED ] );
		$this->assertSame( 'text-embedding-3-small', $this->options[ Dukandaar_Embeddings_Admin::OPT_MODEL ] );
		$this->assertSame( 512, $this->options[ Dukandaar_Embeddings_Admin::OPT_DIMS ] );
		$this->assertSame( 1000, $this->options[ Dukandaar_Embeddings_Admin::OPT_CAP ] );
	}
	...
	public function test_save_persists_provider_type_endpoint_and_keys(): void {
		...
		$this->assertSame( 'cohere', $this->options[ Dukandaar_Embeddings_Admin::OPT_PROVIDER_TYPE ] );
		$this->assertSame( 'https://api.moonshot.ai/v1', $this->options[ Dukandaar_Embeddings_Admin::OPT_BASE_URL ] );
		$this->assertSame( 'sk-moon', $this->options[ Dukandaar_Embeddings_Admin::OPT_API_KEY ] );
		$this->assertSame( 'co-key', $this->options[ Dukandaar_Embeddings_Admin::OPT_COHERE_KEY ] );
	}

	public function test_save_persists_qdrant_settings_with_collection_default(): void {
		Dukandaar_Embeddings_Admin::save( [ 'qdrant_url' => 'https://q.example.com:6333', 'qdrant_key' => 'qk' ] );
		$this->assertSame( 'https://q.example.com:6333', $this->options['dukandaar_qdrant_url'] );
		$this->assertSame( 'qk', $this->options['dukandaar_qdrant_key'] );
		$this->assertSame( 'dukandaar_products', $this->options['dukandaar_qdrant_collection'], 'blank collection -> default' );
	}

	public function test_save_rejects_unknown_provider_type_and_defaults_base_url(): void {
		Dukandaar_Embeddings_Admin::save( [ 'embedding_provider_type' => 'bogus' ] );
		$this->assertSame( 'openai', $this->options[ Dukandaar_Embeddings_Admin::OPT_PROVIDER_TYPE ] );
		$this->assertSame( 'https://api.openai.com/v1', $this->options[ Dukandaar_Embeddings_Admin::OPT_BASE_URL ] );
	}

	public function test_save_clamps_and_defaults(): void {
		Dukandaar_Embeddings_Admin::save( [ 'embedding_dims' => '0', 'embed_daily_cap' => '-5', 'embedding_model' => '' ] );
		$this->assertSame( 0, $this->options[ Dukandaar_Embeddings_Admin::OPT_ENABLED ], 'unchecked box -> disabled' );
		$this->assertGreaterThanOrEqual( 1, $this->options[ Dukandaar_Embeddings_Admin::OPT_DIMS ] );
		$this->assertSame( 0, $this->options[ Dukandaar_Embeddings_Admin::OPT_CAP ], 'negative cap floored to 0' );
		$this->assertSame( 'text-embedding-3-small', $this->options[ Dukandaar_Embeddings_Admin::OPT_MODEL ], 'blank model -> default' );
	}

	public function test_run_build_enqueues_backfill_and_records_index_model(): void {
		$this->options['dukandaar_embeddings_enabled'] = 1;
		$this->options['dukandaar_openai_api_key']     = 'sk-live';
		$enqueued                                     = 0;
		...
		$count = Dukandaar_Embeddings_Admin::run_build();
	...
	public function test_run_build_is_a_noop_without_a_provider(): void {
		...
		$this->assertSame( 0, Dukandaar_Embeddings_Admin::run_build() );
	...
	public function test_enabled_gate_blocks_retrieval_even_with_a_key(): void {
		...
		$this->options['dukandaar_openai_api_key'] = 'sk-live';
		$this->options['dukandaar_embeddings_enabled'] = 0;
		$this->assertFalse( Dukandaar_Embeddings::enabled() );
		$this->assertNull( Dukandaar_Retriever::resolve_seam( null, 'warm', [] ), 'disabled -> keyword fallback' );
	}
}
