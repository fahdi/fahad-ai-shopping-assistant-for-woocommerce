...
class AnalyticsDashboardTest extends TestCase {
	...
	public function test_parse_date_returns_null_for_blank(): void {
		...
		$this->assertNull( dukandaar_analytics_parse_date( '   ', true ) );
	}
	...
	public function test_parse_date_rejects_a_malformed_date(): void {
		...
		$this->assertNull( dukandaar_analytics_parse_date( 'not-a-date', false ) );
		$this->assertNull( dukandaar_analytics_parse_date( '2026/06/18', false ) );
		$this->assertNull( dukandaar_analytics_parse_date( "2026-06-18'; DROP", false ) );
	}

	public function test_parse_date_start_is_midnight_and_end_is_end_of_day(): void {
		$start = dukandaar_analytics_parse_date( '2026-06-18', false );
		$end   = dukandaar_analytics_parse_date( '2026-06-18', true );
	...
	public function test_outcome_label_maps_known_outcomes(): void {
		$this->assertSame( 'Escalated', dukandaar_analytics_outcome_label( Dukandaar_Analytics::OUTCOME_ESCALATED ) );
		$this->assertSame( 'No matching action', dukandaar_analytics_outcome_label( Dukandaar_Analytics::OUTCOME_NO_TOOL_MATCH ) );
	}

	public function test_outcome_label_passes_through_an_unknown_key(): void {
		$this->assertSame( 'mystery', dukandaar_analytics_outcome_label( 'mystery' ) );
	}
	...
	public function test_attribute_orders_defaults_to_zero_without_a_filter(): void {
		...
		$this->assertSame( 0, dukandaar_attribute_orders( [ 'conv-a', 'conv-b' ] ) );
	}
	...
	public function test_attribute_orders_is_filterable(): void {
		...
		$this->assertSame( 2, dukandaar_attribute_orders( [ 'conv-a', 'conv-b' ] ) );
	}
}
