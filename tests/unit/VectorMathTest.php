<?php
/**
 * Unit tests for Dukandaar_Vector_Math (RAG Phase 0, S0.1).
 *
...
class VectorMathTest extends TestCase {
	...
	public function test_pack_unpack_round_trips_arbitrary_values_within_float32_precision(): void {
		...
		$out = Dukandaar_Vector_Math::unpack_vector( Dukandaar_Vector_Math::pack_vector( $v ) );
	...
	public function test_cosine_of_identical_vectors_is_one(): void {
		$v = [ 1.0, 2.0, 3.0 ];
		$this->assertEqualsWithDelta( 1.0, Dukandaar_Vector_Math::cosine( $v, $v ), 1e-9 );
	}

	public function test_cosine_of_orthogonal_vectors_is_zero(): void {
		$this->assertEqualsWithDelta( 0.0, Dukandaar_Vector_Math::cosine( [ 1.0, 0.0 ], [ 0.0, 1.0 ] ), 1e-9 );
	}

	public function test_cosine_of_opposite_vectors_is_minus_one(): void {
		$this->assertEqualsWithDelta( -1.0, Dukandaar_Vector_Math::cosine( [ 1.0, 1.0 ], [ -1.0, -1.0 ] ), 1e-9 );
	}

	public function test_cosine_with_a_zero_vector_is_zero_not_nan(): void {
		$c = Dukandaar_Vector_Math::cosine( [ 0.0, 0.0, 0.0 ], [ 1.0, 2.0, 3.0 ] );
	...
	public function test_cosine_rejects_mismatched_lengths(): void {
		...
		Dukandaar_Vector_Math::cosine( [ 1.0, 2.0 ], [ 1.0, 2.0, 3.0 ] );
	}
}
