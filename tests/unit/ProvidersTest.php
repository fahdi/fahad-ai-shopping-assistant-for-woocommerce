<?php
/**
 * Unit tests for Dukandaar_Providers, the multi-provider catalog.
 *
 ...
 *   - the catalog is filterable via `dukandaar_providers` (the add-on seam),
...
class ProvidersTest extends TestCase {
	...
	public function test_every_preset_declares_the_required_shape(): void {
		foreach ( Dukandaar_Providers::catalog() as $id => $preset ) {
	...
	public function test_openai_default_model_is_gpt_4o_mini(): void {
		$this->assertSame( 'gpt-4o-mini', Dukandaar_Providers::catalog()['openai']['default_model'] );
		$this->assertContains( 'gpt-4o-mini', Dukandaar_Providers::catalog()['openai']['models'] );
	}
	...
	public function test_anthropic_and_moonshot_keep_their_existing_option_names(): void {
		...
		$this->assertSame( 'dukandaar_anthropic_api_key', $catalog['anthropic']['key_option'] );
		$this->assertSame( 'dukandaar_anthropic_model', $catalog['anthropic']['model_option'] );
		$this->assertSame( 'dukandaar_moonshot_api_key', $catalog['moonshot']['key_option'] );
		$this->assertSame( 'dukandaar_moonshot_model', $catalog['moonshot']['model_option'] );
	}
	...
	public function test_new_provider_option_names_follow_the_id_convention(): void {
		// Each NEW preset reads dukandaar_{id}_api_key / dukandaar_{id}_model.
		foreach ( [ 'openai', 'gemini', 'groq', 'deepseek', 'xai', 'custom' ] as $id ) {
			$preset = Dukandaar_Providers::catalog()[ $id ];
			$this->assertSame( "dukandaar_{$id}_api_key", $preset['key_option'], "{$id}: key option" );
			$this->assertSame( "dukandaar_{$id}_model", $preset['model_option'], "{$id}: model option" );
		}
	}

	// ── dukandaar_providers filter (add-on extensibility seam) ────────────────────
	...
	public function test_catalog_is_filterable_to_register_a_new_provider(): void {
		...
		Functions\when( 'apply_filters' )->alias(
			static function ( $hook, $value = null ) {
				if ( 'dukandaar_providers' === $hook && is_array( $value ) ) {
					$value['acme'] = [
						'label'         => 'Acme LLM',
						'type'          => 'openai',
	...
	public function test_filter_is_applied_with_the_documented_hook_name(): void {
		...
		Dukandaar_Providers::catalog();

		$this->assertSame( 'dukandaar_providers', $seen, 'The catalog must run the dukandaar_providers filter.' );
	}
	...
	public function test_get_returns_a_known_preset(): void {
		$preset = Dukandaar_Providers::get( 'openai' );
	...
	public function test_get_returns_null_for_an_unknown_provider(): void {
		$this->assertNull( Dukandaar_Providers::get( 'does-not-exist' ) );
	}

	public function test_ids_lists_every_catalog_provider(): void {
		$ids = Dukandaar_Providers::ids();
		...
		$this->assertSame( array_keys( Dukandaar_Providers::catalog() ), $ids );
	}
	...
	public function test_resolve_reads_key_and_model_for_an_openai_provider(): void {
		$this->set_options( [
			'dukandaar_openai_api_key' => 'sk-openai-123',
			'dukandaar_openai_model'   => 'gpt-4o',
		] );

		$resolved = Dukandaar_Providers::resolve( 'openai' );
	...
	public function test_resolve_defaults_model_to_the_preset_default(): void {
		...
		$this->set_options( [ 'dukandaar_openai_api_key' => 'sk-openai-123' ] );
	...
	public function test_resolve_anthropic_uses_existing_option_names(): void {
		...
		// under dukandaar_anthropic_api_key / model under dukandaar_anthropic_model.
		$this->set_options( [
			'dukandaar_anthropic_api_key' => 'sk-ant-legacy',
			'dukandaar_anthropic_model'   => 'claude-sonnet-4-6',
		] );

		$resolved = Dukandaar_Providers::resolve( 'anthropic' );
	...
	public function test_resolve_moonshot_uses_existing_options_and_region_base_url(): void {
		// BACKWARD COMPAT: moonshot keeps dukandaar_moonshot_api_key / _model AND its
		// region-selected base URL (dukandaar_moonshot_region), unchanged.
		$this->set_options( [
			'dukandaar_moonshot_api_key' => 'sk-moon-legacy',
			'dukandaar_moonshot_model'   => 'kimi-k2.5',
			'dukandaar_moonshot_region'  => 'china',
		] );

		$resolved = Dukandaar_Providers::resolve( 'moonshot' );
	...
	public function test_resolve_moonshot_defaults_to_global_base_url(): void {
		$this->set_options( [ 'dukandaar_moonshot_api_key' => 'sk-moon' ] );
	...
	public function test_resolve_custom_reads_merchant_base_url(): void {
		...
		$resolved = Dukandaar_Providers::resolve( 'custom' );
	...
	public function test_resolve_returns_null_for_an_unknown_provider(): void {
		$this->assertNull( Dukandaar_Providers::resolve( 'nope' ) );
	}
	...
	public function test_resolve_a_filter_registered_provider(): void {
		...
		Functions\when( 'get_option' )->alias(
			static fn( $key, $default = '' ) => 'dukandaar_acme_api_key' === $key ? 'sk-acme' : $default
		);

		$resolved = Dukandaar_Providers::resolve( 'acme' );
	...
	public function test_sanitize_base_url_accepts_https(): void {
		...
		$this->assertSame( 'https://api.example.com/v1', Dukandaar_Providers::sanitize_base_url( 'https://api.example.com/v1' ) );
	}
	...
	public function test_sanitize_base_url_rejects_plain_http_remote_and_junk(): void {
		...
		$this->assertSame( '', Dukandaar_Providers::sanitize_base_url( 'http://api.example.com/v1' ), 'remote http rejected' );
		$this->assertSame( '', Dukandaar_Providers::sanitize_base_url( 'ftp://api.example.com' ), 'ftp rejected' );
		$this->assertSame( '', Dukandaar_Providers::sanitize_base_url( 'javascript:alert(1)' ), 'dangerous scheme rejected' );
		$this->assertSame( '', Dukandaar_Providers::sanitize_base_url( 'garbage' ), 'junk rejected' );
		$this->assertSame( '', Dukandaar_Providers::sanitize_base_url( '' ), 'empty rejected' );
	}
	...
	public function test_sanitize_base_url_allows_localhost_http(): void {
		...
		$this->assertSame( 'http://localhost:11434/v1', Dukandaar_Providers::sanitize_base_url( 'http://localhost:11434/v1' ) );
		$this->assertSame( 'http://127.0.0.1:8080/v1', Dukandaar_Providers::sanitize_base_url( 'http://127.0.0.1:8080/v1' ) );
	}

	// ── type()/is_openai() ────────────────────────────────────────────────────────

	public function test_type_and_is_openai(): void {
		$this->assertSame( 'anthropic', Dukandaar_Providers::type( 'anthropic' ) );
		$this->assertSame( 'openai', Dukandaar_Providers::type( 'openai' ) );
		$this->assertSame( '', Dukandaar_Providers::type( 'nope' ), 'Unknown id has no type.' );

		$this->assertFalse( Dukandaar_Providers::is_openai( 'anthropic' ) );
		$this->assertTrue( Dukandaar_Providers::is_openai( 'openai' ) );
		$this->assertTrue( Dukandaar_Providers::is_openai( 'moonshot' ) );
		$this->assertFalse( Dukandaar_Providers::is_openai( 'nope' ) );
	}
}
