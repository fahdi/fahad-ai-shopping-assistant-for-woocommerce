<?php
/**
 * Unit tests for Dukandaar_Rag_Spike_Retriever (RAG Phase 0, S0.4).
 *
 ...
 *  - hybrid_rank:  RRF fusion of the two (reuses Dukandaar_Rrf)
...
class RagSpikeRetrieverTest extends TestCase {
	...
	public function test_keyword_rank_scores_by_token_overlap_and_excludes_zero(): void {
		...
		$this->assertSame( [ 1, 3 ], Dukandaar_Rag_Spike_Retriever::keyword_rank( 'warm winter', $docs ) );
	}

	public function test_keyword_rank_is_case_insensitive_and_tokenised(): void {
		$docs = [ 5 => 'Warm, WINTER coat!' ];
		$this->assertSame( [ 5 ], Dukandaar_Rag_Spike_Retriever::keyword_rank( 'warm winter', $docs ) );
	}
	...
	public function test_vector_rank_orders_by_cosine_and_excludes_non_candidates(): void {
		...
		$this->assertSame( [ 1, 3 ], Dukandaar_Rag_Spike_Retriever::vector_rank( [ 1.0, 0.0, 0.0 ], $vecs ) );
	}
	...
	public function test_hybrid_rank_recovers_items_each_leg_alone_misses(): void {
		...
		$hybrid = Dukandaar_Rag_Spike_Retriever::hybrid_rank( [ 7 ], [ 8 ] );
	...
	public function test_hybrid_rank_matches_rrf_of_the_legs(): void {
		...
		$vec = [ 3, 2 ];
		$this->assertSame(
			Dukandaar_Rrf::fuse( [ $kw, $vec ] ),
			Dukandaar_Rag_Spike_Retriever::hybrid_rank( $kw, $vec )
		);
	}
}
