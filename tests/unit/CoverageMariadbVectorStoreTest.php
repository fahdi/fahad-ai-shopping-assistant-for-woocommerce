<?php
/**
 * Coverage tests for Dukandaar_MariaDb_Vector_Store, exercises the write paths
...
class CoverageMariadbVectorStoreTest extends TestCase {
	...
	public function test_is_available_false_when_version_unparsable(): void {
		...
		$store = new Dukandaar_MariaDb_Vector_Store( 'm', 3 );
	...
	public function test_upsert_creates_table_and_inserts_vector_literal(): void {
		...
		( new Dukandaar_MariaDb_Vector_Store( 'text-embedding-3-small', 3 ) )
		...
		$this->assertStringContainsString( 'CREATE TABLE IF NOT EXISTS wp_dukandaar_vectors', $queries[0] );
		...
		$this->assertStringContainsString( 'INSERT INTO wp_dukandaar_vectors', $prepared['sql'] );
	...
	public function test_delete_prepares_delete_by_product_id(): void {
		...
		( new Dukandaar_MariaDb_Vector_Store( 'm', 3 ) )->delete( 99 );

		$this->assertSame( 'PREPARED_DELETE', $ran, 'the prepared statement is the one executed' );
		$this->assertStringContainsString( 'DELETE FROM wp_dukandaar_vectors', $prepared['sql'] );
	...
	public function test_content_hash_returns_stored_hash(): void {
		...
		$hash = ( new Dukandaar_MariaDb_Vector_Store( 'm', 3 ) )->content_hash( 7 );

		$this->assertSame( 'deadbeef', $hash );
		$this->assertStringContainsString( 'SELECT content_hash FROM wp_dukandaar_vectors', $prepared['sql'] );
		$this->assertStringContainsString( 'WHERE product_id = %d', $prepared['sql'] );
	...
	public function test_rebuild_required_true_when_index_model_differs(): void {
		...
		$store = new Dukandaar_MariaDb_Vector_Store( 'new-model', 3 );
	...
	public function test_rebuild_required_false_when_index_model_matches(): void {
		...
		$store = new Dukandaar_MariaDb_Vector_Store( 'same-model', 3 );
	...
	public function test_rebuild_required_reads_the_index_model_option(): void {
		...
		$this->assertTrue( $store->rebuild_required() );
		$this->assertSame( Dukandaar_Postmeta_Vector_Store::OPTION_INDEX_MODEL, $seen );
	}
	...
	public function test_upsert_interpolates_configured_dimensions_into_ddl(): void {
		...
		( new Dukandaar_MariaDb_Vector_Store( 'm', 1536 ) )
