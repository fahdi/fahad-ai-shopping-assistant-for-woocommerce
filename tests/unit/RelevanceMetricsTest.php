<?php
/**
 * Unit tests for Dukandaar_Relevance_Metrics (RAG Phase 0, S0.4).
 *
...
class RelevanceMetricsTest extends TestCase {
	...
	public function test_recall_at_k(): void {
		$retrieved = [ 1, 2, 3, 4 ];
		$relevant  = [ 2, 4 ];
		$this->assertSame( 1.0, Dukandaar_Relevance_Metrics::recall_at_k( $retrieved, $relevant, 4 ) );
		// top-2 = [1,2] ∩ {2,4} = {2} → 1/2
		$this->assertSame( 0.5, Dukandaar_Relevance_Metrics::recall_at_k( $retrieved, $relevant, 2 ) );
	}
	...
	public function test_precision_at_k(): void {
		...
		$this->assertSame( 0.5, Dukandaar_Relevance_Metrics::precision_at_k( $retrieved, $relevant, 4 ) );
		// top-2 = [1,2] → 1 relevant of 2 → 0.5
		$this->assertSame( 0.5, Dukandaar_Relevance_Metrics::precision_at_k( $retrieved, $relevant, 2 ) );
	}

	public function test_recall_with_empty_relevant_is_one_vacuously(): void {
		$this->assertSame( 1.0, Dukandaar_Relevance_Metrics::recall_at_k( [ 1, 2 ], [], 2 ) );
	}

	public function test_ndcg_perfect_ranking_is_one(): void {
		// Relevant item at position 1 → DCG == IDCG.
		$this->assertEqualsWithDelta( 1.0, Dukandaar_Relevance_Metrics::ndcg_at_k( [ 2, 1, 3 ], [ 2 ], 3 ), 1e-9 );
	}

	public function test_ndcg_discounts_lower_positions(): void {
		// Single relevant item at position 2 → 1/log2(3) ≈ 0.63093.
		$this->assertEqualsWithDelta( 0.63093, Dukandaar_Relevance_Metrics::ndcg_at_k( [ 1, 2, 3 ], [ 2 ], 3 ), 1e-5 );
	}
	...
	public function test_ndcg_two_relevant_below_ideal(): void {
		...
		$this->assertEqualsWithDelta( 0.69343, Dukandaar_Relevance_Metrics::ndcg_at_k( [ 1, 2, 3 ], [ 2, 3 ], 3 ), 1e-5 );
	}
}
