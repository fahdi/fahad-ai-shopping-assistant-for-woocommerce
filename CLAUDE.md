# Dukandaar AI Shopping Assistant for WooCommerce, index
...
## Identity
- Main file: `dukandaar-ai-shopping-assistant-for-woocommerce.php` · GitHub: https://github.com/fahdi/dukandaar-ai-shopping-assistant-for-woocommerce
- **Current version: 2.14.3**
- Requires: WordPress 6.0+ (tested up to 7.0), PHP 8.0+ (host dev runs 8.5), WooCommerce active
- Slug / text domain (must match): `dukandaar-ai-shopping-assistant-for-woocommerce` (WP.org pending, see #67)
- Prefixes: constants `DUKANDAAR_*`, classes `Dukandaar_*`, functions/hooks/options `dukandaar_*`, REST `dukandaar/v1`. Full conventions table in [docs/plugin.md](docs/plugin.md).
...
3. **Security invariant, don't regress.** The public `/message` + `/stream` endpoints keep the `wp_rest` nonce **and** the per-client rate limit. Personal-data tools add the `Dukandaar_Auth` boundary (login gate + per-record ownership) on top.
...
2. **Bump the version (semver)**, patch for fixes, minor for features, in all four places: the main-file header `Version:`, `DUKANDAAR_VERSION`, `readme.txt` `Stable tag`, and this file's "Current version".
